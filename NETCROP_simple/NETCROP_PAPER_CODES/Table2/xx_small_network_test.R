...
one_simulation <- function(simulation) {
  ...
  if(phase=="NETCROP") for(nrep in c(1L,5L)) {
    ...
    rows[[k]] <- data.frame(simulation,algorithm="NETCROP",model="RDPG",n,d,xi,lambda,max_d=max(d_candidates),loss_function=losses,s=num_subnetworks,o=overlap_size,cv=NA_integer_,R=nrep,d_hat=selected,best_model=paste0("RDPG-",selected),run_time=timing[["elapsed"]])
  ...
  if(phase=="comparison") for(nrep in c(1L,20L)) {
    ...
    print(fit$overall_best,row.names=FALSE)
    rows[[k]] <- data.frame(simulation,algorithm="ECV",model="RDPG",n,d,xi,lambda,max_d=max(d_candidates),loss_function=losses,s=NA_integer_,o=NA_integer_,cv=3L,R=nrep,d_hat=selected,best_model=paste0("RDPG-",selected),run_time=timing[["elapsed"]])
...
tables<-lapply(successful,function(x)x$result$summary);if(length(tables)){cat("\nNETCROP summary:\n");summarize_results(do.call(rbind,tables))};message("\nWARNING: ECV is very slow and memory consuming and may crash R on a personal computer. The quick test will continue automatically.");phase<-"comparison";comparison<-netOP::run_simulations(one_simulation,nsim=nsim,results_file=sub("\\.rds$","_comparison.rds",results_file),action="resume",show_progress=TRUE,continue_on_error=TRUE);tables<-c(tables,lapply(Filter(function(x)isTRUE(x$success),comparison),function(x)x$result$summary));if(length(tables)){results<-do.call(rbind,tables);write.csv(results,csv_file,row.names=FALSE);cat("\nFinal RDPG quick-test summary:\n");summarize_results(results)}
