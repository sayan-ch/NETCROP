...
one_simulation <- function(simulation) {
  ...
  if (phase == "NETCROP") for (nrep in c(1L, 5L)) {
    timing <- system.time(fit <- netOP::netcrop_rdpg(A, d_candidates, num_subnetworks,
      overlap_size, nrep, losses, ncores = ncores, verbose = FALSE))
    ...
    rows[[k]] <- data.frame(simulation, algorithm="NETCROP", model="RDPG", n, d, xi, lambda,
      max_d=max(d_candidates), loss_function=losses, s=num_subnetworks, o=overlap_size,
      cv=NA_integer_, R=nrep, d_hat=selected, best_model=paste0("RDPG-",selected), run_time=timing[["elapsed"]],
  ...
  if (phase == "comparison") for (nrep in c(1L, 20L)) {
    timing <- system.time(fit <- netOP::ecv_stability_rdpg(A, max(d_candidates), 3L,
      nrep, 0.9, losses, ncores = ncores, verbose = FALSE))
    selected <- fit$overall_best$d_hat[fit$overall_best$loss == losses]
    ...
    rows[[k]] <- data.frame(simulation, algorithm="ECV", model="RDPG", n, d, xi, lambda,
      max_d=max(d_candidates), loss_function=losses, s=NA_integer_, o=NA_integer_,
      cv=3L, R=nrep, d_hat=selected, best_model=paste0("RDPG-",selected), run_time=timing[["elapsed"]],
      user_time=timing[["user.self"]], system_time=timing[["sys.self"]])
