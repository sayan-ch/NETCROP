...
one_simulation <- function(simulation) {
  ...
  if (phase == "NETCROP") for (nrep in c(1L, 5L)) {
    ...
    rows[[k]] <- data.frame(simulation, algorithm = "NETCROP", model = "RDPG", n, d, xi, lambda,
                            ...
                            d_hat = selected, best_model = paste0("RDPG-", selected), run_time = timing[["elapsed"]],
...
summarize_results <- function(results) {
  groups <- split(results, interaction(results$model, results$algorithm, results$R, drop = TRUE))
  out <- do.call(rbind, lapply(groups, function(x) {
    counts <- sort(table(x$best_model), decreasing = TRUE)
    data.frame(model=x$model[1L], algorithm=x$algorithm[1L], R=x$R[1L],
      best_model=names(counts)[1L], selected_count=as.integer(counts[1L]),
      selected_percent=100*as.integer(counts[1L])/nrow(x),
      accuracy=100*mean(x$d_hat==x$d), MAD=mean(abs(x$d_hat-x$d)))
  }))
  print(out, row.names = FALSE); invisible(out)
}
records <- netOP::run_simulations(one_simulation, nsim = nsim, results_file = sub("\\.rds$", "_netcrop.rds", results_file),
...
message("\nWARNING: ECV is very slow and memory consuming on this network and may crash R on a personal computer.")
proceed <- TRUE
if (interactive()) proceed <- tolower(trimws(readline("Proceed with ECV? [y/N]: "))) %in% c("y", "yes")
if (proceed) {
  phase <- "comparison"
  comparison <- netOP::run_simulations(one_simulation, nsim=nsim,
    results_file=sub("\\.rds$", "_comparison.rds", results_file), action="resume",
    show_progress=TRUE, continue_on_error=TRUE)
  tables <- c(tables, lapply(Filter(function(x)isTRUE(x$success), comparison), function(x)x$result$summary))
} else message("ECV was skipped; NETCROP results remain saved.")
if(length(tables)){results<-do.call(rbind,tables);write.csv(results,csv_file,row.names=FALSE);cat("\nFinal RDPG summary:\n");summarize_results(results)}
