...
one_simulation <- function(simulation) {
  ...
  for (i in seq_along(c(1L, 5L))) {
    ...
    rows[[i]] <- data.frame(simulation, model="LSM", n, d, alpha, lambda, max_d=max(d_candidates), loss_function=losses,
...
if (length(successful)) {
  ...
  print(aggregate(cbind(lambda, run_time, correct=as.numeric(d_hat==d), d_hat, absolute_error=abs(d_hat-d)) ~ s+o+R, results, function(x) mean(x, na.rm=TRUE)))
}
