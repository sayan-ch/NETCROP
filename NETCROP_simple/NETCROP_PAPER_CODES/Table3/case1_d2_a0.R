...
run_name <- "case1_d2_a0"; output_dir <- file.path(script_dir, "output", run_name)
...
nsim <- 100L; n <- 1000L; d <- 2L; alpha <- 0; d_candidates <- 1:5; losses <- "sse"
...
one_simulation <- function(simulation) {
  A <- netOP::generate_lsm(n=n, d=d, alpha=alpha, ncores=ncores)
  lambda <- mean(Matrix::rowSums(A)); rows <- list(); fits <- list()
  for (i in seq_along(c(1L, 5L))) {
    nrep <- c(1L, 5L)[[i]]
    timing <- system.time(fit <- netOP::netcrop_lsm(A, d_candidates, num_subnetworks, overlap_size, nrep, losses,
      lsm_options=list(step_size=0.3, niter=100L, trace=FALSE), ncores=ncores, verbose=FALSE))
    selected <- fit$overall_best$d_hat[fit$overall_best$loss == losses]
    message(sprintf("Simulation %d/%d | NETCROP R=%d | per-repetition d_hat=%s | elapsed=%.3f s", simulation, nsim, nrep,
      paste(fit$best_dimension_cv$d_hat, collapse=","), timing[["elapsed"]]))
    print(fit$overall_best, row.names=FALSE)
    rows[[i]] <- data.frame(simulation, model="LSM", algorithm="NETCROP", n, d, alpha, lambda, max_d=max(d_candidates), loss_function=losses,
      s=num_subnetworks, o=overlap_size, R=nrep, d_hat=selected, run_time=timing[["elapsed"]],
      best_model=paste0("LSM-",selected), user_time=timing[["user.self"]], system_time=timing[["sys.self"]])
...
if (length(successful)) {
  ...
  groups<-split(results,interaction(results$model,results$algorithm,results$R,drop=TRUE));summary_table<-do.call(rbind,lapply(groups,function(x){counts<-sort(table(x$best_model),decreasing=TRUE);data.frame(model=x$model[1L],algorithm=x$algorithm[1L],R=x$R[1L],best_model=names(counts)[1L],selected_count=as.integer(counts[1L]),selected_percent=100*as.integer(counts[1L])/nrow(x),accuracy=100*mean(x$d_hat==x$d),MAD=mean(abs(x$d_hat-x$d)))}));print(summary_table,row.names=FALSE)
}
