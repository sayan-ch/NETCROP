...
run_name <- "case2_d2_a1"; output_dir <- file.path(script_dir, "output", run_name); dir.create(output_dir, recursive=TRUE, showWarnings=FALSE)
...
nsim <- 100L; n <- 1000L; d <- 2L; alpha <- 1; d_candidates <- 1:5; losses <- "sse"
...
one_simulation <- function(simulation) {
  ...
  for (i in seq_along(c(1L, 5L))) {
    ...
    rows[[i]] <- data.frame(simulation, model="LSM", algorithm="NETCROP", n, d, alpha, lambda, max_d=max(d_candidates), loss_function=losses, s=num_subnetworks, o=overlap_size,
      R=nrep, d_hat=selected, best_model=paste0("LSM-",selected), run_time=timing[["elapsed"]], user_time=timing[["user.self"]], system_time=timing[["sys.self"]])
