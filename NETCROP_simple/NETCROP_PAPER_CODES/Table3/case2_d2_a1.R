...
one_simulation <- function(simulation) {
  ...
  for (i in seq_along(c(1L, 5L))) {
    ...
    rows[[i]] <- data.frame(simulation, model="LSM", n, d, alpha, lambda, max_d=max(d_candidates), loss_function=losses, s=num_subnetworks, o=overlap_size,
      R=nrep, d_hat=selected, run_time=timing[["elapsed"]], user_time=timing[["user.self"]], system_time=timing[["sys.self"]])
