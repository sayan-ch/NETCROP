...
results_file <- file.path(output_dir,"small_results_reporting.rds"); csv_file <- file.path(output_dir,"small_results.csv")
...
one_simulation <- function(simulation) {
  ...
  for(i in seq_along(c(1L,5L))) {
    ...
    rows[[i]] <- data.frame(simulation,model="LSM",algorithm="NETCROP",n,d,alpha,lambda,max_d=max(d_candidates),loss_function=losses,s=num_subnetworks,o=overlap_size,R=nrep,d_hat=selected,best_model=paste0("LSM-",selected),run_time=timing[["elapsed"]])
