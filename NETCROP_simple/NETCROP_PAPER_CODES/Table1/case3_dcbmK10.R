# Table 1, case 3: DCBM, n = 10000, K = 10.
...
output_dir <- file.path(script_dir, "output", "case3_dcbmK10")
...
nsim <- 100L
n <- 10000L; K <- 10L; ratio <- 0.2; alpha <- 0.5; max_K <- 20L
...
one_simulation <- function(simulation) {
  cat(sprintf("\nTable 1 case 3 | simulation %d/%d | generating DCBM\n", simulation, nsim))
  ...
  A <- generate_dcbm(n = n, K = K, community_probabilities = rep(1 / K, K),
                     alpha = 1, beta = ratio, psi = psi, average_degree = 80,
  ...
  if (phase == "NETCROP") for (r in c(1L, 5L)) run_method("NETCROP", r) else {
    for (r in c(1L, 20L)) run_method("NCV", r); for (r in c(1L, 20L)) run_method("ECV", r)
  }
...
records_netcrop <- run_simulations(one_simulation, nsim = nsim,
  results_file = file.path(output_dir, "case3_dcbmK10_netcrop.rds"), action = "resume",
...
if(proceed){phase<-"comparison";records_comparison<-run_simulations(one_simulation,nsim=nsim,results_file=file.path(output_dir,"case3_dcbmK10_comparison.rds"),action="resume",show_progress=TRUE,continue_on_error=TRUE);tables<-c(tables,Filter(Negate(is.null),lapply(records_comparison,function(x)if(isTRUE(x$success))x$result$data else NULL)))}else message("NCV and ECV were skipped; NETCROP results remain saved.")
if(length(tables)){results<-do.call(rbind,tables);utils::write.csv(results,file.path(output_dir,"case3_dcbmK10.csv"),row.names=FALSE);cat("\nFinal Table 1 case 3 summary:\n");summarize_results(results)}
