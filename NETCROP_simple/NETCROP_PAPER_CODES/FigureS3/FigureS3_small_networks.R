# Figure S3: runtime, R-managed memory, and selection accuracy on small networks.
...
output_dir <- file.path(script_dir, "output", "FigureS3_small_networks")
...
run_configuration <- function(n, model, configuration) {
  ...
  run_simulations(one_simulation, nsim = nsim, use_parallel_simulations = TRUE,
    ncores_outer = ncores_outer, results_file = file, action = "resume",
