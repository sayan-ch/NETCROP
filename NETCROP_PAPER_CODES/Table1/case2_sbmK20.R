...
for(sim in 1:nsim){
  ...
  for(RR in seq_along(R)){
    ...
    readr::write_csv(
      ncv.tab, file = file.path(paste0("output/case2_ncv_v", version, ".csv")),
      append = file.exists(paste0("output/case2_ncv_v", version, ".csv"))
    )

    all.ncv[[count]] <- list(time = time.ncv, out = out.ncv)

    saveRDS(
      all.ncv[[count]], file = file.path(paste0("logs/case2_ncv_v", version, ".rds"))
    )
...
View(nc.all |> dplyr::group_by(s, o, R) |>
       dplyr::summarize(
         ...
         accu = 100 * mean(best_model == "SBM-20"),
...
ncv.all <- readr::read_csv(file.path(paste0("output/case2_ncv_v", version, ".csv")))
