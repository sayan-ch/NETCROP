...
LSM.gen <- function (n, d, K = 1, alpha = NULL, avg.d = NULL, 
  ...
  G <- Z %*% t(Z)
  theta <- Rfast::Outer(alpha, alpha, '+') + G
  P <- 1/(1 + exp(-theta))
  if (!is.null(avg.d)) {
    for (i in 1:10) {
...
Rcpp::sourceCpp(
  fle.path(here::here("NETCROP_PAPER_CODES/helpers/LSM_PGD_Cpp.cpp")))

pgd.lsm <- function (A, d, step.size = 0.3, niter = 500, trace = 0){
  N <- nrow(A)
  Jmat <- diag(N) - 1/N
  
  P.tilde <- randnet::USVT(A)
  P.tilde[P.tilde > (1 - 1e-06)] <- (1 - 1e-06)
  P.tilde[P.tilde < 1e-06] <- 1e-06
  
  Theta.tilde <- log(P.tilde / (1 - P.tilde))
  alpha_0 <- solve(diag(N, N) + 1, rowSums(Theta.tilde))
  
  G <- Jmat %*% (Theta.tilde - Rfast::Outer(alpha_0, alpha_0, "+")) %*% Jmat
...
netcrop_lsm <- function(A, d.cand, s, o, R,
  ...
  match.out <- mclapply(1:nrow(raw.ind), function(ii){
    ...
    proc.par <- Procrustes(raw.out[[ii]]$Z.hat[(1:o), , drop = F],
                           raw.out[[stand]]$Z.hat[(1:o), , drop = F],
                           translate = T,
                           dilate = F)
  ...
  L.all <- lapply(1:nrow(non.mat), function(ii){
    ...
    theta.hat <- Rfast::Outer(beta1.hat, beta2.hat, '+') + Z1.hat %*% t(Z2.hat)
