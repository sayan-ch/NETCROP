# Synthetic smoke test for the block-model procedures used in the Table 4
# analyses. This does not reproduce either real-data result.
...
for (sim in nc.simulations) {
  net <- DCBM.gen(n = n, K = K, avg.deg = 30, beta = 0.3,
  ...
  for (R.use in R) {
    ...
    netcrop_status(sim, nsim, "NETCROP", timing[3],
                   c(result$l2.model, result$AUC.model), R.use)
    readr::write_csv(tibble::tibble(
      nsim = sim, n = n, K = K, max_K = max.K, p_test = p.test,
      s = s, o = o, R = R.use, best_l2 = result$l2.model,
      best_AUC = result$AUC.model, run_time = timing[3]
...
message("Small Table 4 smoke test completed. Results: ",
